...
def build_media_part(file_obj, mime_type: Optional[str]) -> Any:
    """Build an inline media part from Streamlit UploadedFile (avoids Files API tuple/path issues on cloud runtimes)."""
    file_obj.seek(0)
    data = file_obj.read()
    if not data:
...
def run_validation(client: genai.Client, uploaded_files: List[Any], user_context: str, niche: str, compare_to_user: bool) -> Tuple[Dict[str, Any], str]:
    ...
    # Build multimodal contents list: prompt + inline media parts
