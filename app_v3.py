...
6) Cite sources used for each validated claim (title + URL). Prefer primary / high-trust sources.
6.1) Keep output compact: at most 6 claims total and at most 3 sources per claim.
...
def build_media_part(file_obj, mime_type: Optional[str]) -> Any:
    """Build an inline media part from Streamlit UploadedFile (stable on Streamlit Cloud)."""
    file_obj.seek(0)
    data = file_obj.read()
    if not data:
...
def _escape_control_chars_inside_strings(s: str) -> str:
    """Escape illegal control chars that appear inside JSON strings."""
    out = []
    in_str = False
    escaped = False
    for ch in s:
...
def _sanitize_suspicious_embedded_json_in_urls(s: str) -> str:
    # If model accidentally injects ```json ... inside a URL string, truncate URL at the fence marker.
    return re.sub(
        r'("url"\s*:\s*"[^"]*?)```json.*?(?=")',
        r"\1",
        s,
...
def safe_json_parse(text: str) -> Tuple[Optional[Dict[str, Any]], Optional[str]]:
    ...
    - Cuts off trailing garbage/duplication
    - Sanitizes unescaped ASCII control chars that break json.loads
    """
    if not text:
        return None, "Empty model response"

    s = _strip_fences(text)
    s = _sanitize_suspicious_embedded_json_in_urls(s)
    ...
    # First escape illegal control chars inside quoted strings, then normalize remaining controls outside strings.
    s = _escape_control_chars_inside_strings(s)
        ...
        for i, ch in enumerate(s):
            if ch == "{":
                ...
                if depth == 0:
                    ...
                    break
        if end_idx is None:
            return None, f"JSON parse failed: could not match braces. Error: {e}. Raw starts: {text[:300]}"
        candidate = s[:end_idx]
        candidate = _escape_control_chars_inside_strings(candidate)
        candidate = _sanitize_suspicious_embedded_json_in_urls(candidate)
...
def run_validation(
    ...
    for f in uploaded_files:
        ...
        media_parts.append(build_media_part(f, mime))
    ...
    # grounding metadata presence (for debugging)
    try:
...
def main():
    ...
    if run_btn:
        ...
        for idx, f in enumerate(files, start=1):
            ...
            try:
                res, _ = run_validation(
                    client, [f], user_context=user_context, niche=niche, compare_to_user=compare_to_user
                )
